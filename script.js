...
let bookmarks = []; // IDs of saved channels (Still LocalStorage for user privacy)
...
let userLikes = []; // LocalStorage for user specific likes
let collections = []; // Array of objects { id, name, channelIds } (Local)
...
async function init() {
    // Show loading state
    grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:2rem"><i class="fa-solid fa-spinner fa-spin fa-2x"></i><br>Connecting to Cloud...</div>';
...
async function loadData() {
    // 1. Load Local User Data (Bookmarks, UserLikes, Collections) - Privacy
    ...
    // 2. Fetch Public Data from Firebase (Channels, Stats, Global Config)
    try {
        const snapshot = await db.ref('/').once('value');
        const data = snapshot.val() || {};

        channels = data.channels || window.INITIAL_DATA || [];
        likes = data.likes || {};
        ...
        if (config.announcement) {
            ...
            if (b) {
                b.innerHTML = escapeHtml(config.announcement);
        ...
        // Fallback to initial data if offline or empty
        channels = window.INITIAL_DATA || [];
        grid.innerHTML = '<div style="grid-column:1/-1; text-align:center; color:orange">Offline Mode / Config Missing</div>';
    }
}

async function saveData(type) {
    // We split save into Global (Firebase) and Local (Storage)
    ...
    // Global Save (Only specific parts based on action)
    // For MVP we just push the modified big objects, but ideally we use 'update'

    if (type === 'likes') {
        await db.ref('likes').set(likes);
    }
    if (type === 'comments') {
        await db.ref('comments').set(comments);
    }
    // We generally don't save 'channels' from client unless it's admin.
...
window.toggleLike = function (id) {
    ...
    saveData('likes'); // Persist globally
...
window.openInfoModal = function (id) {
    ...
    const related = channels
        .filter(x => x.category === c.category && x.id !== c.id) // Same cat, not self
    ...
    if (related.length > 0) {
        relatedHtml = `
            <div style="margin-top:2rem; border-top:1px solid var(--glass-border); padding-top:1rem; text-align:left;">
                <h4 style="margin-bottom:1rem; color:var(--text-secondary)">You Might Also Like</h4>
                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap:0.5rem">
                    ${related.map(r => {
            const rLogo = r.logo && !r.logo.startsWith('data:') ? r.logo : generateAutoLogo(r.link, r.name);
            return `
                            <div onclick="openInfoModal('${r.id}')" style="background:var(--secondary-bg); padding:0.5rem; border-radius:8px; cursor:pointer; text-align:center; border:1px solid var(--glass-border)">
                                <img src="${rLogo}" style="width:40px; height:40px; border-radius:8px; display:block; margin:0 auto 0.5rem" onerror="this.src='${generateAutoLogo(r.link, r.name)}'">
                                <div style="font-size:0.75rem; font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis">${escapeHtml(r.name)}</div>
                            </div>
                         `;
        }).join('')}
                </div>
...
document.getElementById('submit-form').addEventListener('submit', async (e) => {
    ...
    // Validation
    const bl = ["scam", "casino", "hack", "porn", "xxx"];
    ...
    // Save to pending (FIREBASE)
    // We append to a 'pending' list in DB
    try {
        const pRef = db.ref('pending');
        const newItemRef = pRef.push(); // Generate new key
        await newItemRef.set(newChannel);

        closeModal('submit-modal');
        document.getElementById('submit-form').reset();
        alert('Submitted to Cloud! Waiting for admin approval.');
    } catch (e) {
        console.error(e);
        alert("Submission failed. Check connectivity.");
    }
});
...
document.getElementById('report-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        alert('Report submitted to Cloud.');
    } catch (e) {
...
document.getElementById('verification-form').addEventListener('submit', async (e) => {
    ...
    try {
        await db.ref('verification_requests').push(req);
        closeModal('verification-modal');
        alert('Verification request sent to Cloud!');
    } catch (e) {
...
window.openComments = function (id) {
    ...
    // Comments are now object with keys because of Firebase Push? 
    // If we map from Firebase, it comes as Object usually.
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
    ...
    // path: comments / channelId / pushId
    await db.ref(`comments/${window.currentChannelId}`).push(newComment);

    // Optimistic Update? Or just wait for reload? 
    // For simplicity, we just reload data or push to local cache
    // Let's re-fetch? No, too heavy.
    // We update local cache manually
    if (!comments[window.currentChannelId]) comments[window.currentChannelId] = {};
    // Hack to display immediately:
    const tempKey = 'temp_' + Date.now();
    comments[window.currentChannelId][tempKey] = newComment;
